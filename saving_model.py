import pickle
from log_code import setup_logging
logger = setup_logging("saving_model")

def saving(reg,X_train):
    try:
        with open("car_price.pkl", "wb") as f:
            pickle.dump(reg, f)
        logger.info('saving_model')
        with open("car_price.pkl","rb") as t:
            m=pickle.load(t)
        logger.info(f'testing the own data prediction :{m.predict(X_train.tail(1))}')
    except Exception as e:
        er_ty, er_msg, er_line = sys.exc_info()
        logger.info(f'error in line no:{er_line.tb_lineno} due to : {er_msg} and reason was {er_ty}')

